...
const DeleteSubChem = ({isOpen , onClose , selectedSubItem , currentDatabase, handleRefresh}) => {
  const [confirmation,setConfirmation] = useState('')
    if (!isOpen || !selectedSubItem) return null;
    console.log('on deletesub')
    const onDelete = async () => {
      if (confirmation !== 'CONFIRM') {
        ...
        const deleteResponse = await fetch(`https://api.airtable.com/v0/appNRC5vSBgrKCqii/${currentDatabase}/${recordId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': 'Bearer patuAn2pKiuFSMoI8.1ad68d143585a93ed0c2348b3ab3adb9c1f8364b814d1a9149f763b6087ef2f3',
            'Content-Type': 'application/json'
...
return (
  <Popup isOpen={isOpen} onDismiss={onClose} verticalOffset={30}>
    <View style={styles.container}>
      ...
      <Text style={styles.DeleteText}>Delete {selectedSubItem.ItemNo} from {currentDatabase}?</Text>
      <TextInput
          style={styles.input}
